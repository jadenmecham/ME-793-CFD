...
%
Introduce the lid-driven cavity problem and its role as a standard benchmark in
incompressible CFD~\citep{ghia1982}. Briefly explain why it is nontrivial (recirculating
flow, corner vortices, Re-dependent topology) and what makes the fractional-step
method~\citep{perot1993} an effective approach for incompressible flows.
...
%
% Describe:
%   - Uniform staggered grid: u at horizontal faces, v at vertical faces, p at cell centers.
%   - Grid sizes: u is (nx-1)xny, v is nx x(ny-1), p is nx x ny.
%   - Pinned pressure reference (bottom-left cell, p=0).
%   - Include a schematic figure if possible.
%
Describe the staggered grid layout with velocity components defined at cell faces
and pressure at cell centers. State the grid dimensions for each variable and
the pressure reference condition.

\begin{figure}[t]
  \centering
  %\includegraphics[width=0.85\linewidth]{figs/staggered_grid.pdf}
  % Replace with your own staggered grid schematic
